...
int main (void) {
	printf("========================================\n");
	printf("Test: Insert, Remove\n");
	Set s;
	createEmptySet(&s);

	showOutput("empty set = ", &s);
	insertSet(&s, 99);
	showOutput("add 99 = ", &s);
	insertSet(&s, 98);
	showOutput("add 98 = ", &s);
	...
	showOutput("add 3 = ", &s);
	insertSet(&s, 100);
	showOutput("add 100 = ", &s);

	removeSet(&s, 0);
	showOutput("remove 0 = ", &s);
	removeSet(&s, 0);
	showOutput("remove 0 = ", &s);
	removeSet(&s, 3);
	showOutput("remove 3 = ", &s);
	removeSet(&s, 100);
	showOutput("remove 100 = ", &s);
	removeSet(&s, 1);
	showOutput("remove 1 = ", &s);
	...
	insertSet(&s, 10);
	showOutput("Set: ", &s);
	bool is_member = isMemberSet(&s, 1);
	if(is_member){
		printf("1 is in the set\n");
	}
	else {
		printf("1 is not in the set\n");
	}
	is_member = isMemberSet(&s, 2);
	if(is_member){
	...
	is_member = isMemberSet(&s, 10);
	if(is_member){
		printf("10 is in the set\n");
	}
	else {
		printf("10 is not in the set\n");
	}
	is_member = isMemberSet(&s, 11);
	if(is_member){
		printf("11 is in the set\n");
	}
	else {
		printf("11 is not in the set\n");
	}
	is_member = isMemberSet(&s, 0);
	if(is_member){
	}
	else {
		printf("0 is in the set\n");
		printf("0 is not in the set\n");
	}
	destroySet(&s);
	printf("========================================\n");
	...
	insertSet(&s, 10);
	showOutput("Set S:", &s);
	showOutput("Subset T: ", &t);
	bool is_subset = isSubsetOf(&t, &s);
	if(is_subset){
...
void checkCase(setFun fun, Set* s1, Set* s2, Set* expect) {
	...
	//showOutput("S1: ", &res);						////my debug line
	//showOutput("S2: ", s2);							////my debug line
	(*fun)(&res, s2);
	//showOutput("mine: ", &res);						////my debug line
	//showOutput("expected: ", expect);				////my debug line
	//printf("\n");									////my debug line
...
void specialCaseTests(void) {
	...
	for (i = 0; i < number_of_tests; i += 1) {
	  ...
	  //showOutput("subset: ",&s);					//debug
	  //showOutput("universal: ",&universal);			//debug
