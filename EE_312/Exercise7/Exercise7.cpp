...
void problem1(void) {
	...
	Complex y{4.2}; // Initialize y using Constructor #2
	Complex z{0, -10.0}; // Initialize z using Constructor #3
 ...
 *
 		7 destructor print messages..
			5 third strings destroyed
			1 second string
			1 first string
 *
 *
 ...
 * doubleDeleteBugOhMy creates a double delete because:
 	...
 	there are are 2 copies of the string objects. 1 in doubledelete's scope, and one in problem 3's scope..
	they both contain a pointer pointin to the same location on the heap.
	As a result, there will be 2 calls to free on the same space in heap.
 ...
 * not suffer from the same double-delete as function 1
	...
	There is no object in noBugWhyNot()'s scope.. as a result the string object is only destructed once
	when problem3() returns.
 ...
 * When function 3 is called, is there also be a double delete? If so, why? If not, why not?
	...
	The underlying implementation of references are just pointers.. So again there is no copy of
	the string object and no double calls to freeing the same location.
...
void Complex::operator+=(Complex y) { real = real + y.real;
										imag = imag + y.imag; }
void Complex::operator-=(Complex y) { real = real - y.real;
										imag = imag - y.imag; }
...
void problem5(void) {
	...
	anArrayOfFoo[0].doit4(anArrayOfFoo + 10);			//   <= this is so hacky.
