...
kirby = pygame.image.load("kirby_1_r.png")
tile_pic = pygame.image.load("tile_1.png")
...
while (state != EXIT):
    ...
    if(state == INIT):
        ...
        if(action == DIG):
            ...
            if(xindex>=0 and yindex>=0):
                ...
                while (i< num_tiles):
                    a,b = tiles[i].get_pos()
                    if(a==xindex and b == yindex):
                        tiles[i].dig() ##error here, thinks tiles[i] is a bool type ##edit fixed... prob was there was a field called dig and a func named dig()
                        if(tiles[i].is_mine()):
                            state = LOSE
                        break
                    i = i+1
    ...
    elif(action == EXITGAME or state == EXIT):
